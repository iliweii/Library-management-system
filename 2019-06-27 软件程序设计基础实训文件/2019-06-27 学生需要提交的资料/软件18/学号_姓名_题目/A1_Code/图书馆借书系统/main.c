...
void AccountManage() // 用户账号管理函数
{
    ...
    if(USER[chooseNumber].status == 0)
    {
        ...
        case 1:
        {
            ...
            for(i = 0; i < UserNum; i++)
            {
                fputs(USER[i].username, fp);
                fputs("\n", fp);
                fputs(USER[i].password, fp);
                fputs("\n", fp);
                fputs(USER[i].age, fp);
                fputs("\n", fp);
                fputs("1", fp);
    ...
    if(resultNumber == 1)
    {
        ...
        else if(chooseNumber == 1)     // 允许用户借阅这本书
        {
            ...
            else
            {
                ...
                printf("\n\t%c 借阅成功！  借阅时间：%d-%02d-%02d %d:%d:%d", BorderLeft, lt->tm_year+1900, lt->tm_mon+1, lt->tm_mday, lt->tm_hour, lt->tm_min, lt->tm_sec);
                printf("\n\t\t");
            }
        }
...
void BookLendStatus() // 图书借阅状态
{
    ...
    int ki = 0;
    for(ki = 0; ki < LendNum; ki++)
    {
        if(strcmp(LEND[ki].userName, userCookie.username) == 0)
        {
            for(j = 0; j < BookNum; j++)
            {
                if(strcmp(LEND[ki].bookNum, BOOK[j].bookNum) == 0)
                {
                    ...
                    char StartTime[55];
                    strcpy(StartTime, LEND[ki].startTime);
                    int time1 = Timestamp(timeStr);
                    int time2 = Timestamp(StartTime);
                    printf("\n|<-已借阅：%d天->\n", time1-time2);
                }
            }
        }
...
void BookReturn() // 图书归还
{
    SystemOp("cls");
    int time1, time2;
    int chooseNumber, lendNumber, lendDeleteNum, resultNumber;
    char borderChar = RandomNumber(BORDER, sizeof(BORDER)/sizeof(BORDER[0]));
    char searcherStr[55];
    BOOKLEND tempLend;
        ...
        if(strcmp(LEND[ki].userName, userCookie.username) == 0)
        {
            for(j = 0; j < BookNum; j++)
            {
                if(strcmp(LEND[ki].bookNum, BOOK[j].bookNum) == 0)
                {
                    ...
                    char StartTime[55];
                    strcpy(StartTime, LEND[ki].startTime);
                    time1 = Timestamp(timeStr);
                    time2 = Timestamp(StartTime);
                    printf("\n<-已借阅：%d天->\n", time1-time2);
                    lendDeleteNum = ki;
                }
            }
    ...
    {
        printf("\n\t%c 请输入想要归还的书号：", BorderLeft);
        gets(searcherStr);
        for(k = 0; k < LendNum; k++)
        {
            if(StringMatch(LEND[k].bookNum, searcherStr) == 1 && strcmp(LEND[k].userName, userCookie.username) == 0)
            {
                lendDeleteNum = k;
                resultNumber++;
            }
    ...
    {
        char StartTime[55];
        strcpy(StartTime, LEND[lendDeleteNum].startTime);
        time2 = Timestamp(StartTime);
        printf("\n\t%c 该书已经借阅 %d天", BorderLeft , time1-time2);
        printf("\n\t%c 是否归还 书号：%s这本书？(0 取消| 1 确认)：", BorderLeft, LEND[lendDeleteNum].bookNum);
        chooseNumber = ChooseVerify(0, 1);
        if(chooseNumber == 0)   // 取消
        {
            BookReturn(); // 返回图书归还函数
        ...
        else if(chooseNumber == 1)     // 归还
        {
            if(time1-time2 > 30)
            {
                printf("\n\t *该书借阅时间为%d天, *", time1-time2);
                printf("\n\t *清先联系管理员缴纳逾期费用 *");
                printf("\n\t *感谢使用图书馆借书系统*");
                printf("\n\t *ESC 键返回主菜单*");
                chooseNumber = ChooseVerify(0, 0);
            ...
            tempLend = LEND[LendNum-1];
            LEND[LendNum-1] = LEND[lendDeleteNum];
            LEND[lendDeleteNum] = tempLend;
            LendNum--;
            // 重新写入文件
            FILE *fp = NULL; // 打开文件 booklend.txt
            fp = fopen("booklend.txt", "w+");
            for(i = 0; i < LendNum; i++)
            {
                fputs(LEND[i].bookNum, fp);
                fputs("\n", fp);
                fputs(LEND[i].userName, fp);
                fputs("\n", fp);
                fputs(LEND[i].startTime, fp);
                fputs("\n", fp);
            }
            fclose(fp); // 关闭文件
            printf("\n ");
